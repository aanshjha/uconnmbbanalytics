...
When local private inputs are complete, the default run copies only the cached ESPN responses, manual game CSVs, and two core game/stint CSVs into a temporary directory. It rebuilds canonical data, forecast evaluation, and the Florida review there, checks reconciliation flags, all team-stat comparisons, finite forecast errors, and that no human sessions were invented, then removes the temporary directory. It does not fetch from the network or modify saved project outputs. `--require-real-data` makes missing inputs a failure; the default reports a skip when they are absent.
...
Syntax and fixture checks establish that inspected code parses and behaves as specified on test cases. The isolated real-data run additionally verifies that cached source totals reconcile in the current workflow. None of these checks verifies five-player attribution, possession boundaries in the legacy stint tables, forecasting improvement over the baseline, or staff usefulness. Current data findings and release limits are documented in [the reliability report](../docs/RELIABILITY_RESET.md).
